...
func MkTree(SuperBlock Estructuras.Sblock, dsk *os.File) string {
	...
	DatosArch += "<TR>\n<TD BGCOLOR=\"lightblue\">I_atime</TD><TD>" + fmt.Sprint(InodoRoot.I_atime) + " </TD>\n</TR>\n"
...
func TreeBlckCarpetas(pos int64, dsk *os.File, SuperBlock Estructuras.Sblock) string {
	...
	DatosArch += "<TR>\n<TD BGCOLOR=\"lightblue\">Nombrfe</TD><TD>Pos</TD>\n</TR>\n"
	for i := 0; i < 4; i++ {
		act := bloqueCarpeta.B_content[i]
		DatosArch += "<TR>\n<TD BGCOLOR=\"lightblue\">" + fmt.Sprint(act.B_name[:]) + "</TD><TD>" + fmt.Sprint(act) + " </TD>\n</TR>\n"
	}
	DatosArch += "</TABLE>>];\n"

	for _, i := range bloqueCarpeta.B_content {
		pos2 := i.B_inodo
		DatosArch += "B" + fmt.Sprint(pos) + "->" + "I" + fmt.Sprint(pos2)
...
func TreeInodoArch(pos int64, dsk *os.File, SuperBlock Estructuras.Sblock) string {
	...
	DatosArch += "<TR>\n<TD BGCOLOR=\"lightblue\">I_atime</TD><TD>" + fmt.Sprint(InodoArchivo.I_atime) + " </TD>\n</TR>\n"
	...
	if string(InodoArchivo.I_type) == "1" {
		...
		for i := 0; i < 16; i++ {
			...
			if (int64(act) - 1) != -1 {
				...
				DatosArch += TreeBlckCarpetas((int64(i) - 1), dsk, SuperBlock)
			}
		}
	}
